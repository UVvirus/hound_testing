...
contract QuillTest is ERC20, Ownable {
    ...
    {   
       ...
       // _mint(msg.sender, 1e9 * (10 ** decimals()));
        ...
        //_approve(address(this), address(_uniswapV2Router.WETH()), type(uint256).max);
        ...
        _approve(_msgSender(), address(uniswapV2Router), type(uint256).max);
        uniswapV2Router.addLiquidity(address(this), _uniswapV2Router.WETH(),100,100,50,50,_msgSender(),block.timestamp);
        liquidityFeeOnBuy  = 1;
        ...
        _mint(_msgSender(), 1e9 * (10 ** decimals()));
